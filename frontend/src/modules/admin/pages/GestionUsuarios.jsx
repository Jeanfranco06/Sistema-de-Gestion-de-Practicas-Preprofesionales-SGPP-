...
export const GestionUsuarios = () => {
    ...
    const validateEmail = (email, rolPrincipal) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const { confirmPassword, rolPrincipal, codigoMatricula, semestre, codigoDocente, categoria, especialidad, departamento, empresaNombre, cargo, area, ...dataToSend } = formData;
    ...
    const handleUnlock = async (id) => {
        try { await usuariosApi.unlock(id); loadUsuarios(); } catch (error) { MySwal.fire('Error', 'No se pudo desbloquear', 'error'); }
    };
    ...
    const handleSaveRoles = async () => {
        try {
            ...
            loadUsuarios();
        } catch (error) { MySwal.fire('Error', 'Error al actualizar roles', 'error'); }
    };
    ...
    if (initialLoad) {
        return (
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh', flexDirection: 'column', gap: 3 }}>
                ...
                <Typography variant="body1" color="text.secondary" fontWeight={500}>Cargando directorio de usuarios...</Typography>
            </Box>
        );
    }
        ...
        <Box sx={{ px: { xs: 1.5, sm: 2, md: 2.5 }, py: { xs: 2, md: 4 }, width: '100%', pb: 8 }}>
                ...
                <Box>
                    <Paper elevation={0} sx={{ mb: 4, borderRadius: { xs: 3, md: 4 }, overflow: 'hidden', bgcolor: '#1a365d', color: 'white', display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', md: 'center' }, p: { xs: 3, md: 5 }, gap: { xs: 4, md: 3 }, position: 'relative' }}>
                        ...
                        <Box sx={{ position: 'relative', zIndex: 1, width: '100%' }}>
                            ...
                            <Typography variant="h3" fontWeight={800} sx={{ mt: 0, mb: 1.5, fontSize: { xs: '1.75rem', sm: '2rem', md: '2.5rem' }, wordBreak: 'break-word' }}>Gestión de Usuarios</Typography>
                                        ...
                                        <TableRow key={usuario.id} hover sx={{ '&:last-child td': { border: 0 } }}>
                                            <TableCell>
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                                    ...
                                                    <Box>
                                                        <Typography variant="body2" fontWeight={700} color="text.primary">@{usuario.username}</Typography>
                                                        <Typography variant="caption" color="text.secondary" fontWeight={600} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>{usuario.tipoUsuario}</Typography>
                                                    </Box>
                                                </Box>
                                            </TableCell>
                                            <TableCell>
                                                <Typography variant="body2" fontWeight={600} color="text.primary">{`${usuario.nombres} ${usuario.apellidoPaterno}`}</Typography>
                                            ...
                                            <TableCell><Typography variant="body2" color="text.secondary" fontWeight={500}>{usuario.email}</Typography></TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                                                        ...
                                                        <Typography variant="caption" fontWeight={700} color={usuario.activo ? '#10b981' : '#ef4444'}>{usuario.activo ? 'ACTIVO' : 'INACTIVO'}</Typography>
                                                    </Box>
                                    ...
                                    {sortedUsuarios.length === 0 && !loading && (
                                        <TableRow>
                                            <TableCell colSpan={6} align="center" sx={{ py: 8 }}>
                                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1, color: '#94a3b8' }}><SearchIcon sx={{ fontSize: 48, opacity: 0.5 }} /><Typography variant="subtitle1" fontWeight={600}>No se encontraron resultados</Typography></Box>
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth slotProps={{ paper: { sx: { borderRadius: 4, overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' } } }}>
                <DialogTitle sx={{ bgcolor: '#1a365d', color: '#fff', py: 2.5, px: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <BadgeIcon /> <Typography variant="h6" fontWeight={700}>{isEditing ? 'Editar Usuario' : 'Registrar Nuevo Usuario'}</Typography>
                    </Box>
                ...
                <DialogContent sx={{ p: { xs: 2, md: 5 }, bgcolor: '#fff', minHeight: 350 }}>
                    ...
                    {activeStep === 0 && (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                            <Typography variant="subtitle1" fontWeight={700} color="text.secondary">¿Qué tipo de usuario deseas crear?</Typography>
                                    ...
                                    <Grid item xs={12} sm={6} md={4} key={rol}>
                                        ...
                                        >
                                            <CardActionArea onClick={() => { if (!isEditing) setFormData({ ...formData, rolPrincipal: rol }); }} sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                                ...
                                                <Typography fontWeight={700} color={formData.rolPrincipal === rol ? 'text.primary' : 'text.secondary'}>
                                                    {rol.replace(/_/g, ' ')}
                                                </Typography>
                                            </CardActionArea>
                                        </Card>
                    ...
                    {activeStep === 1 && (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                            <Typography variant="h6" fontWeight={700} color="text.primary" sx={{ mb: 1 }}>Datos Personales y Contacto</Typography>
                    ...
                    {activeStep === 2 && (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                            <Typography variant="h6" fontWeight={700} color="text.primary" sx={{ mb: 1 }}>Perfil: {effectiveFormRole.replace(/_/g, ' ')}</Typography>
                    ...
                    {activeStep === 3 && (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                            <Typography variant="h6" fontWeight={700} color="text.primary" sx={{ mb: 1 }}>Credenciales de Acceso</Typography>
                                    ...
                                    {(!isEditing || formData.password) && (
                                        <Box sx={{ mt: 1 }}>
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                                ...
                                                <Typography variant="caption" fontWeight={700} color={calculatePasswordStrength(formData.password) < 50 ? 'error' : calculatePasswordStrength(formData.password) < 75 ? 'warning.main' : 'success.main'}>
                    ...
                    {activeStep === 4 && (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                            <Box sx={{ textAlign: 'center', mb: 2 }}>
                                ...
                                <Typography variant="h5" fontWeight={800} color="text.primary">Casi listo</Typography>
                            ...
                            <Paper variant="outlined" sx={{ borderRadius: 3, overflow: 'hidden' }}>
                                <Box sx={{ p: 2, bgcolor: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between' }}>
                                    <Typography variant="subtitle2" fontWeight={700}>Rol: {effectiveFormRole.replace(/_/g, ' ')}</Typography>
                                ...
                                <Box sx={{ p: 3, display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2 }}>
                                    <Box><Typography variant="caption" color="text.secondary" display="block">Nombre Completo</Typography><Typography variant="body2" fontWeight={600}>{formData.nombres} {formData.apellidoPaterno} {formData.apellidoMaterno}</Typography></Box>
                                    <Box><Typography variant="caption" color="text.secondary" display="block">Documento ({formData.tipoDocumento})</Typography><Typography variant="body2" fontWeight={600}>{formData.numeroDocumento}</Typography></Box>
                                    <Box><Typography variant="caption" color="text.secondary" display="block">Correo Institucional / Contacto</Typography><Typography variant="body2" fontWeight={600}>{formData.email}</Typography></Box>
                                    <Box><Typography variant="caption" color="text.secondary" display="block">Nombre de Usuario (Login)</Typography><Typography variant="body2" fontWeight={700} color="#1a365d">@{formData.username}</Typography></Box>
                                </Box>
                            </Paper>
                        </Box>
                    )}
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: '#f8fafc' }}>
                        <Box sx={{ p: 3, bgcolor: '#1a365d', color: 'white', display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                            ...
                            <Box sx={{ flexGrow: 1 }}>
                                ...
                                <Typography variant="h5" fontWeight={800} sx={{ mt: -0.5, mb: 2 }}>{selectedUsuario.nombres} {selectedUsuario.apellidoPaterno}</Typography>
                        ...
                        <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 3 }}>
                            <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid #e2e8f0', mb: 3 }}>
                                <Typography variant="subtitle2" color="primary.main" fontWeight={700} sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}><PersonIcon fontSize="small" /> Información General</Typography>
                                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                                    <Box><Typography variant="caption" color="text.secondary" fontWeight={600}>Usuario</Typography><Typography variant="body2" fontWeight={700}>@{selectedUsuario.username}</Typography></Box>
                                    <Box><Typography variant="caption" color="text.secondary" fontWeight={600}>Documento ({selectedUsuario.tipoDocumento})</Typography><Typography variant="body2" fontWeight={600}>{selectedUsuario.numeroDocumento}</Typography></Box>
                                    <Box sx={{ gridColumn: '1 / -1' }}><Typography variant="caption" color="text.secondary" fontWeight={600}>Correo Electrónico</Typography><Typography variant="body2" fontWeight={600}>{selectedUsuario.email}</Typography></Box>
                                    <Box sx={{ gridColumn: '1 / -1' }}><Typography variant="caption" color="text.secondary" fontWeight={600}>Teléfono</Typography><Typography variant="body2" fontWeight={600}>{selectedUsuario.telefono || 'No registrado'}</Typography></Box>
                                </Box>
                            </Paper>
                            <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid #e2e8f0', mb: 3 }}>
                                <Typography variant="subtitle2" color="primary.main" fontWeight={700} sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}><AdminPanelSettingsIcon fontSize="small" /> Roles Asignados</Typography>
                            ...
                            {selectedUsuario.tipoUsuario === 'INTERNO' && (selectedUsuario.estudiante || selectedUsuario.codigoEstudiantil || selectedUsuario.codigoMatricula || selectedUsuario.estudiante?.codigoEstudiantil) && (
                                <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid #bfdbfe', bgcolor: '#eff6ff', mb: 3 }}>
                                    <Typography variant="subtitle2" color="#1e40af" fontWeight={700} sx={{ mb: 2 }}>Perfil Académico (Estudiante)</Typography>
                                    <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                                        <Box><Typography variant="caption" color="#60a5fa" fontWeight={600}>Código Matrícula</Typography><Typography variant="body2" color="#1e3a8a" fontWeight={700}>{selectedUsuario.estudiante?.codigoEstudiantil || selectedUsuario.estudiante?.codigoMatricula || selectedUsuario.estudiante?.codigoEstudiante || selectedUsuario.estudiante?.matricula || selectedUsuario.codigoEstudiantil || selectedUsuario.codigoMatricula || selectedUsuario.codigoEstudiante || selectedUsuario.matricula || '—'}</Typography></Box>
                                        <Box><Typography variant="caption" color="#60a5fa" fontWeight={600}>Semestre Actual</Typography><Typography variant="body2" color="#1e3a8a" fontWeight={700}>{selectedUsuario.estudiante?.semestreActual || selectedUsuario.estudiante?.semestre || selectedUsuario.estudiante?.ciclo || selectedUsuario.semestre || selectedUsuario.ciclo || '—'}</Typography></Box>
                                        <Box><Typography variant="caption" color="#60a5fa" fontWeight={600}>Créditos Aprobados</Typography><Typography variant="body2" color="#1e3a8a" fontWeight={700}>{selectedUsuario.estudiante?.creditosAprobados || '—'}</Typography></Box>
                                    </Box>
                                </Paper>
                            )}
                            {selectedUsuario.tutorExterno && (
                                <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid #bbf7d0', bgcolor: '#f0fdf4', mb: 3 }}>
                                    <Typography variant="subtitle2" color="#166534" fontWeight={700} sx={{ mb: 2 }}>Perfil Empresarial (Tutor Externo)</Typography>
                                    <Box sx={{ display: 'grid', gridTemplateColumns: '1fr', gap: 2 }}>
                                        <Box><Typography variant="caption" color="#4ade80" fontWeight={600}>Empresa</Typography><Typography variant="body2" color="#14532d" fontWeight={700}>{selectedUsuario.tutorExterno.empresaNombre}</Typography></Box>
                                        <Box><Typography variant="caption" color="#4ade80" fontWeight={600}>Cargo / Área</Typography><Typography variant="body2" color="#14532d" fontWeight={700}>{selectedUsuario.tutorExterno.cargo} {selectedUsuario.tutorExterno.area ? `(${selectedUsuario.tutorExterno.area})` : ''}</Typography></Box>
                                    </Box>
                                </Paper>
                            )}
                        </Box>
            ...
            <Dialog open={openRolDialog} onClose={() => setOpenRolDialog(false)} maxWidth="sm" fullWidth slotProps={{ paper: { sx: { borderRadius: 4, overflow: 'hidden' } } }}>
                <DialogTitle sx={{ bgcolor: '#f8fafc', color: '#1e293b', py: 2.5, display: 'flex', alignItems: 'center', gap: 1.5, borderBottom: '1px solid #e2e8f0' }}><AdminPanelSettingsIcon color="primary" /> <Typography variant="h6" fontWeight={700}>Gestionar Permisos</Typography></DialogTitle>
