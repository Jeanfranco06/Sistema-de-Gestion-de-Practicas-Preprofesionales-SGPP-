-- 1. Agregar restricción única en la columna nombre de la tabla rubrica
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'unique_rubrica_nombre'
    ) THEN
        ALTER TABLE rubrica ADD CONSTRAINT unique_rubrica_nombre UNIQUE (nombre);
    END IF;
END $$;

-- 2. Agregar campos para unidad de evaluación, horas y tipo de calificación a la tabla evaluacion
ALTER TABLE evaluacion 
ADD COLUMN IF NOT EXISTS unidad VARCHAR(20),
ADD COLUMN IF NOT EXISTS horas_registradas INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS ruta_constancia VARCHAR(500),
ADD COLUMN IF NOT EXISTS tipo_calificacion VARCHAR(20) DEFAULT 'VIGESIMAL';

-- 3. Criterios de evaluación para Tutor Externo (competencias de Ingeniería Industrial)
...
-- 4. Criterios de evaluación para Docente Asesor
INSERT INTO criterio_evaluacion (codigo, nombre, descripcion, puntaje_maximo, tipo_evaluador, activo, creado_por)
